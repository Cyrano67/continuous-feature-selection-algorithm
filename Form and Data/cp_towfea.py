...
for i in range(7,21):
    ...
    # 分母提取
    # fenmu = 0
    ny = pd.read_excel("log_rep.xlsx", usecols=[i])
    nyy = ny.values
    nn = use.dTl(ny)
    ...
    for j in range(1,21):
        # 标签数据提取
        nyse1 = pd.read_excel("absolute_psd.xlsx", usecols=[j])
        d = use.dTl(nyse1)
        ...
        l2.append(h)
    l.append(l2)
wb = use.format_excel("r&a_h", np.array(l))
wb.save("re$ab_condition_h.xlsx")
