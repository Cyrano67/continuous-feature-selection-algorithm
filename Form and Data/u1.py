...
def condition_HXD(d, a, fenmu):
    ...
    for j in lD:
        ...
        print("at: ",at)
        print("h: ",h)
        log_pad = kde.KernelDensity(kernel='gaussian', bandwidth=h).fit(aa2t).score_samples(aa2)  # log px|d
        # print("log_pxd",log_pad)
        tmp = len(at) * np.exp(log_pad)  # p d|x 的分子
        # print("p",tmp)
    ...
    for k in range(0, len(log_pad)):
        nlog_pad = np.array(log_pad[k])
        npda = np.array(pdx[k])
